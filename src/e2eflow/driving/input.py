...
def _read_flow(filenames, num_epochs=None):   #   这里每一种dataset都不一样，还得具体再改
...
class DrivingInput(Input):
    ...
    def _input_flow(self):
        flow_dir = os.path.join(self.data.current_dir, 'driving_frames_finalpass/flow')# 这里要改成flow！不是用这个 是.flo文件的，不是图片(frame)
    ...
    def input_train_gt(self, hold_out):
        img_dirs = ['data_sf/driving_frames_finalpass/training',
                    'data_sf/driving_frames_finalpass/fine_tune_training',
                    'data_sf/driving_frames_finalpass/test',
                    'data_sf/driving_frames_finalpass/fine_tune_test']
        gt_dirs = ['pengfei/LI XINYAO/driving_frames_finalpass_flo/training_flow',
                   'pengfei/LI XINYAO/driving_frames_finalpass_flo/fine_tune_training_flow',
                   'pengfei/LI XINYAO/driving_frames_finalpass_flo/test_flow',
                   'pengfei/LI XINYAO/driving_frames_finalpass_flo/fine_tune_test_flow']
    ...
    def input_train_unsuper(self):
        return self._input_train('driving_frames_finalpass/training')

    def input_train_ft(self):
        return self._input_train('driving_frames_finalpass/fine_tune_training')

    def input_test_unsuper(self):
        input_shape, im1, im2 = self._input_images('driving_frames_finalpass/test')
        return tf.train.batch(
           [im1, im2, input_shape],
           batch_size=self.batch_size,
           num_threads=self.num_threads,
    ...
    def input_test_ft(self):
        input_shape, im1, im2 = self._input_images('driving_frames_finalpass/fine_tune_test')
        return tf.train.batch(
           [im1, im2, input_shape],
           batch_size=self.batch_size,
           num_threads=self.num_threads,
