...
class MonkaaData(Data):
    MONKAA_URL = 'https://lmb.informatik.uni-freiburg.de/data/SceneFlowDatasets_CVPR16/Release_april16/data/Monkaa/raw_data/monkaa__frames_finalpass.tar' # download dataset from Internet

    dirs = ['monkaa_frames_finalpass']
    ...
    def _fetch_if_missing(self):
        local_path = os.path.join(self.data_dir, 'monkaa_frames_finalpass')
        if not os.path.isdir(local_path):
            self._download_and_extract(self.MONKAA_URL, local_path)
    ...
    def get_raw_dirs(self):
        ...
        # for folder in ['15mm_focallength/scene_backwards', '15mm_focallength/scene_forwards','35mm_focallength/scene_backwards', '35mm_focallength/scene_forwards']:
            top_dir = os.path.join(self.current_dir, 'data_sf/monkaa_frames_finalpass/' + folder)
